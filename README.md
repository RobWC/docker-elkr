...
docker run --restart=always -d --volumes-from esdata --name elasticsearch elasticsearch elasticsearch -Des.node.name="ESNode"

#logstash central
docker run --restart=always -d --link elasticsearch:es --name logstash_central -p 514:5514 -p 514:5514/udp logstash /docker-entrypoint.sh logstash -e "input { syslog { port => 5514 type => syslog } } output { stdout { } elasticsearch { host => \"es\" }}"
